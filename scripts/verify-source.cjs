...
const projectPersistence = read("electron/project-persistence.cjs")
const projectImport = read("electron/project-import.cjs")
const electronSources = fs.readdirSync(path.join(root, "electron"))
    .filter((name) => name.endsWith(".cjs"))
    .map((name) => read(`electron/${name}`))
    .join("\n")
...
if (!projectPersistence.includes("writeFileSafely(options.outputPath")) fail("project saves must stage before replacing the destination")
if (!projectPersistence.includes("writeStoredZip(stagedOutputPath")) fail("project saves must use the bounded streaming ZIP writer")
if (!projectImport.includes('require("yauzl")')) fail("hostile Project reads must stay on the lazy yauzl boundary")
if (/require\(["']adm-zip["']\)/.test(electronSources) || electronSources.includes("extractAllTo")) fail("production Project archive paths must not use adm-zip extraction or whole-file writing")
