...
assert.equal(Object.isFrozen(INTERFACE_SCALE_VALUES), true)
assert.equal(Reflect.set(INTERFACE_SCALE_VALUES, 0, 80), false)
...
function verifyConcurrentDelivery(order) {
    ...
    const lowerSubscription = lower.subscribe(() => undefined)
    const higherSubscription = higher.subscribe(() => undefined)
...
const recoveryHigherSubscription = recoveryHigher.subscribe(() => undefined)
const recoveryLowerSubscription = recoveryLower.subscribe(() => undefined)
...
recoveryHigherSubscription()
recoveryLowerSubscription()
...
function verifyReentrantMutation({ initial, outer, nested }) {
    ...
    const peerSubscription = peer.subscribe(() => undefined)
    let reentered = false
    subject.subscribe(() => {
        if (reentered) return
        reentered = true
...
// Promise: construction is render-pure. A StrictMode-discarded adapter does not leak a
// storage listener, and the committed subscription owns exactly one listener lifecycle.
const strictEvents = new FakeStorageEvents()
const discarded = createBrowserPresentationAdapter({ storage: new FakeStorage(), events: strictEvents, writerId: "4444444444444444" })
const committed = createBrowserPresentationAdapter({ storage: new FakeStorage(), events: strictEvents, writerId: "5555555555555555" })
assert.equal(strictEvents.listeners.size, 0)
const removeCommitted = committed.subscribe(() => undefined)
assert.equal(strictEvents.listeners.size, 1)
removeCommitted()
assert.equal(strictEvents.listeners.size, 0)
const removeRemounted = committed.subscribe(() => undefined)
assert.equal(strictEvents.listeners.size, 1)
committed.setInterfaceScale(135)
assert.equal(committed.getSnapshot().interfaceScale, 135)
removeRemounted()
assert.equal(strictEvents.listeners.size, 0)
discarded.dispose()
committed.dispose()
