...
try {
    ...
    assert.equal(full.headers["access-control-allow-origin"], "*", "anonymous renderer media must remain readable for origin-clean poster capture")
    ...
    assert.equal(largeStream.headers["access-control-allow-origin"], "*", "streamed media must remain origin-clean for canvas poster capture")
