...
for (const fixtureName of ["one","two","awkward-seven","ordinary-eight","many-24","alpha","failed"]) {
  ...
  for (const timeMs of [0, timeline.durationMs * 0.37, timeline.durationMs]) {
    ...
    assert(frame.cards.filter((card) => card.visible).length <= 9, `visible mount bound ${fixtureName}`);
  }
}
pass("fixture-resource-bounds", { visibleLimit: 9 });
...
if (SCENE_ID === "coverflow-gallery") {
  ...
  const afterWrapEvent = wrap.events.find((event) => event.type === "transition" && event.fromFocus === 12 && event.toFocus === 13);
  const beforeWrap = at(wrap, many, eventSample(wrap, wrapEvent, 0.999));
  const afterWrap = at(wrap, many, eventSample(wrap, afterWrapEvent, 0.001));
