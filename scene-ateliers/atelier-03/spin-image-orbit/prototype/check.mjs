...
check("panoramic identity and readable front/shoulders",()=>{const hold=evaluate({items:FIXTURES.six,stage:CANVASES.wide,controls:DEFAULTS,timeMs:DURATION_MS*.45}),front=[...hold.cards].sort((a,b)=>b.depth-a.depth)[0],shoulders=hold.cards.filter(c=>c.zone==="shoulder"),xs=hold.cards.map(c=>c.x),ys=hold.cards.map(c=>c.y);expect(front.sourceIndex===DEFAULTS.featuredIndex,"featured source missed front gate");expect(front.containerOpacity>.999&&front.artworkOpacity===1&&front.artworkFilter==="none"&&front.cameraFacing,"front not readable");expect(shoulders.length>=2&&shoulders.every(c=>c.containerOpacity>.55&&c.cameraFacing),"shoulders not readable");expect(Math.max(...xs)-Math.min(...xs)>(Math.max(...ys)-Math.min(...ys))*1.3,"silhouette is not panoramic");const b=box(front);expect(b.minX>=-.05&&b.maxX<=CANVASES.wide.width+.05&&b.minY>=-.05&&b.maxY<=CANVASES.wide.height+.05,"front gate clipped");observations.spans={x:Math.max(...xs)-Math.min(...xs),y:Math.max(...ys)-Math.min(...ys)}})
check("large window fills ellipse and swaps at rear guard",()=>{const many=evaluate({items:FIXTURES.many127,stage:CANVASES.wide,controls:{...DEFAULTS,featuredIndex:64},timeMs:DURATION_MS*.45}),xs=many.cards.map(c=>c.x),depths=many.cards.map(c=>c.depth);expect(Math.max(...xs)-Math.min(...xs)>many.effective.ellipseWidth*.82,"large window bunches at front");expect(Math.max(...depths)-Math.min(...depths)>many.effective.zMax*1.85,"large window lacks rear depth");let prior=evaluate({items:FIXTURES.many127,stage:CANVASES.wide,controls:{...DEFAULTS,featuredIndex:64},timeMs:0}),changes=0;for(let i=1;i<=1400;i++){const next=evaluate({items:FIXTURES.many127,stage:CANVASES.wide,controls:{...DEFAULTS,featuredIndex:64},timeMs:DURATION_MS*i/1400}),before=new Set(prior.cards.map(c=>c.id)),after=new Set(next.cards.map(c=>c.id));if(!equal([...before],[...after])){const changed=[...prior.cards.filter(c=>!after.has(c.id)),...next.cards.filter(c=>!before.has(c.id))];expect(changed.every(c=>c.containerOpacity<.02||(c.frontness<.04&&c.guardOpacity<.15)),"window swap left rear guard");changes++}prior=next}expect(changes>0,"large window never advanced");observations.windowChanges=changes})
check("supported front geometry remains in stage",()=>{const corners=[{},...Object.entries(CONTROL_BOUNDS).filter(([k])=>!["featuredIndex","fixedDurationMs"].includes(k)).flatMap(([k,[a,b]])=>[{[k]:a},{[k]:b}])];for(const stage of Object.values(CANVASES))for(const patch of corners){const out=evaluate({items:FIXTURES.mixed,stage,controls:{...DEFAULTS,...patch,featuredIndex:2},timeMs:DURATION_MS*.45}),front=[...out.cards].sort((a,b)=>b.depth-a.depth)[0],b=box(front);expect(b.minX>=-.05&&b.maxX<=stage.width+.05&&b.minY>=-.05&&b.maxY<=stage.height+.05,`front clipped at ${stage.width}x${stage.height}`)}})
check("five scene controls are causal",()=>{for(const [k,v]of[["ellipseWidth",105],["ellipseHeight",55],["planeYaw",-20],["planePitch",29],["cardSize",27]])expect(!equal(snapshot(FIXTURES.six,CANVASES.wide,DEFAULTS,DURATION_MS*.45),snapshot(FIXTURES.six,CANVASES.wide,{...DEFAULTS,[k]:v},DURATION_MS*.45)),`${k} inert`)})
...
check("bounded-many panoramic phrase brings every source through the active window", () => {
  const controls = { ...DEFAULTS }
  const durationMs = compileTimeline(controls).durationMs
  const seen = new Set()
  for (let index = 0; index < 4096; index += 1) {
    const output = evaluate({ items: FIXTURES.many127, stage: CANVASES.wide, controls, timeMs: index / 4096 * durationMs })
    for (const card of output.cards) seen.add(card.sourceIndex)
  }
  expect(seen.size === 127, `panoramic phrase skipped ${127 - seen.size} source identities`)
  observations.fullSourceCoverage = seen.size
})

const result = { scene: "spin-image-orbit", passed: failures.length === 0, checks: vectors.length + 18, failures, observations }
