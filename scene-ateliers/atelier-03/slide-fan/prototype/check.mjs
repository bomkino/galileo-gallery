...
check("source fidelity and resource ceiling", () => {
  ...
  expect(output.resources.mounted <= 12, `mounted ${output.resources.mounted} cards`)
...
check("default overview reads as an open fan, not a pile", () => {
  const overview = evaluate({
    items: FIXTURES.five,
    stage: CANVASES.wide,
    controls: { ...DEFAULTS, paperBreath: 0 },
    timeMs: DURATION_MS * 0.24,
  })
  const bounds = overview.cards.map(fanBounds)
  const unionWidth = Math.max(...bounds.map((value) => value.maxX)) - Math.min(...bounds.map((value) => value.minX))
  const widestCard = Math.max(...overview.cards.map((card) => card.width * card.scale))
  const angleSpan = Math.max(...overview.cards.map((card) => card.angleDeg)) - Math.min(...overview.cards.map((card) => card.angleDeg))
  expect(unionWidth / widestCard >= 2.3, `default overview spans only ${(unionWidth / widestCard).toFixed(3)} card widths`)
  expect(angleSpan >= 125, `default overview opens only ${angleSpan} degrees`)
  expect(overview.effective.safetyCapActive === false, "default wide overview is silently safety-capped")
  observations.defaultOverview = { angleSpan, unionWidthRatio: unionWidth / widestCard, safetyCapActive: overview.effective.safetyCapActive }
})

check("hinge, lift return, finale and readable bounds", () => {
  const controls = { ...DEFAULTS, paperBreath: 0 }
  const baseIn = evaluate({ items: FIXTURES.five, stage: CANVASES.wide, controls, timeMs: DURATION_MS * 0.315 })
  const baseOut = evaluate({ items: FIXTURES.five, stage: CANVASES.wide, controls, timeMs: DURATION_MS * 0.625 })
  const selectedIn = baseIn.cards.find((card) => card.sourceIndex === DEFAULTS.featuredIndex)
  const selectedOut = baseOut.cards.find((card) => card.sourceIndex === DEFAULTS.featuredIndex)
  expect(selectedIn.bottomX === selectedOut.bottomX && selectedIn.bottomY === selectedOut.bottomY && selectedIn.scale === selectedOut.scale, "featured card did not return exactly")
  const corners = [{}, ...Object.entries(CONTROL_BOUNDS).filter(([key]) => !["featuredIndex","fixedDurationMs"].includes(key)).flatMap(([key,[min,max]]) => [{ [key]: min }, { [key]: max }])]
  for (const stage of Object.values(CANVASES)) for (const fixture of [FIXTURES.two,FIXTURES.five,FIXTURES.mixed,FIXTURES.twelve]) for (const patch of corners) for (const phase of [0.24,0.47,0.76]) {
    const output = evaluate({ items: fixture, stage, controls: { ...DEFAULTS, ...patch, featuredIndex: Math.min(DEFAULTS.featuredIndex,fixture.length-1) }, timeMs: DURATION_MS * phase })
    for (const card of output.cards) {
      const bounds = fanBounds(card)
      expect(bounds.minX >= -0.05 && bounds.maxX <= stage.width + 0.05 && bounds.minY >= -0.05 && bounds.maxY <= stage.height + 0.05, `${card.id} clipped at ${stage.width}x${stage.height}`)
    }
  }
  const many = evaluate({ items: FIXTURES.many127, stage: CANVASES.wide, controls: { ...DEFAULTS, featuredIndex: 64 }, timeMs: DURATION_MS * 0.76 })
  expect(many.cards.some((card) => card.sourceIndex === many.finaleIndex), "windowed finale source is not mounted")
  expect(many.finaleIndex === many.window.start + many.window.count - 1, "windowed finale silently targets an off-window source")
})
check("all five controls are causal", () => {
  const cases = [
    ["spreadAngle", 110, 0.24, FIXTURES.five], ["hingeHeight", 72, 0.24, FIXTURES.five],
    ["cardSize", 30, 0.24, FIXTURES.five], ["featuredLift", 24, 0.47, FIXTURES.five],
    ["paperBreath", 1.1, 0.26, FIXTURES.five],
  ]
  for (const [key,value,phase,items] of cases) {
    const baseline = snapshot(items,CANVASES.wide,{...DEFAULTS},DURATION_MS*phase)
    const changed = snapshot(items,CANVASES.wide,{...DEFAULTS,[key]:value},DURATION_MS*phase)
    expect(!equal(baseline,changed), `${key} is inert`)
  }
})
...
const result = { scene: "slide-fan", passed: failures.length === 0, checks: vectors.length + 18, failures, observations }
