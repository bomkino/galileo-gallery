...
check("hostile input rejected before geometry", () => {
  ...
  for (const name of controlNames) {
    for (const value of [Number.NaN, Number.POSITIVE_INFINITY, Number.NEGATIVE_INFINITY]) {
      let rejected = false
      try { evaluate({ items: FIXTURES.seven, stage: CANVASES.wide, controls: { ...DEFAULTS, [name]: value }, timeMs: 0 }) } catch { rejected = true }
  ...
  try { evaluate({ items: FIXTURES.seven, stage: { width: Number.NaN, height: 720 }, controls: DEFAULTS, timeMs: 0 }) } catch { stageRejected = true }
...
check("source fidelity and resource ceiling", () => {
  ...
  expect(output.resources.mounted <= 16, `mounted ${output.resources.mounted} cards`)
...
check("nonlinear near passage is bounded and visibly nonuniform",()=>{const samples=[.10,.15,.20,.25,.30,.35].map(p=>evaluate({items:FIXTURES.seven,stage:CANVASES.wide,controls:DEFAULTS,timeMs:DURATION_MS*p}).turn),steps=samples.slice(1).map((v,i)=>v-samples[i]),positive=steps.filter(v=>v>1e-6);expect(Math.max(...positive)/Math.min(...positive)>1.8,"approach is effectively uniform");const near=evaluate({items:FIXTURES.seven,stage:CANVASES.wide,controls:DEFAULTS,timeMs:DURATION_MS*.42}),front=[...near.cards].sort((a,b)=>b.depth-a.depth)[0],rear=[...near.cards].sort((a,b)=>a.depth-b.depth)[0];expect(front.sourceIndex===DEFAULTS.featuredIndex,"featured source missed near gate");expect(front.scale>rear.scale*1.25,"near/far scale contrast is weak");expect(front.scale<=front.safeScaleCap+1e-6,"near card exceeded safety cap");observations.approachSteps=steps})
check("large source window fills the full path and swaps at far guard",()=>{const many=evaluate({items:FIXTURES.many127,stage:CANVASES.wide,controls:{...DEFAULTS,featuredIndex:64},timeMs:DURATION_MS*.42}),xs=many.cards.map(c=>c.x),depths=many.cards.map(c=>c.depth);expect(Math.max(...xs)-Math.min(...xs)>many.effective.orbitRadius*1.85,"large window bunches at front");expect(Math.max(...depths)-Math.min(...depths)>many.effective.depthRadius*1.85,"large window lacks far pass");let prior=evaluate({items:FIXTURES.many127,stage:CANVASES.wide,controls:{...DEFAULTS,featuredIndex:64},timeMs:0}),changes=0;for(let i=1;i<=1400;i++){const next=evaluate({items:FIXTURES.many127,stage:CANVASES.wide,controls:{...DEFAULTS,featuredIndex:64},timeMs:DURATION_MS*i/1400}),before=new Set(prior.cards.map(c=>c.id)),after=new Set(next.cards.map(c=>c.id));if(!equal([...before],[...after])){const changed=[...prior.cards.filter(c=>!after.has(c.id)),...next.cards.filter(c=>!before.has(c.id))];expect(changed.every(c=>c.containerOpacity<.02||(c.frontness<.04&&c.guardOpacity<.15)),"window swap occurred near camera");changes++}prior=next}expect(changes>0,"large window never advanced");observations.windowChanges=changes})
check("supported geometry remains in stage",()=>{const corners=[{},...Object.entries(CONTROL_BOUNDS).filter(([k])=>!["featuredIndex","fixedDurationMs"].includes(k)).flatMap(([k,[a,b]])=>[{[k]:a},{[k]:b}])];for(const stage of Object.values(CANVASES))for(const patch of corners){const out=evaluate({items:FIXTURES.mixed,stage,controls:{...DEFAULTS,...patch},timeMs:DURATION_MS*.42});for(const c of out.cards){const b=box(c);expect(b.minX>=-.05&&b.maxX<=stage.width+.05&&b.minY>=-.05&&b.maxY<=stage.height+.05,`${c.id} clipped`)}}})
check("four scene controls are causal",()=>{for(const [k,v]of[["orbitRadius",48],["proximityStrength",15],["pathProjection",82],["cardSize",29]])expect(!equal(snapshot(FIXTURES.seven,CANVASES.wide,DEFAULTS,DURATION_MS*.42),snapshot(FIXTURES.seven,CANVASES.wide,{...DEFAULTS,[k]:v},DURATION_MS*.42)),`${k} inert`)})


const REVIEW_FEATURED_KEY = "featuredIndex"
const REVIEW_CONTROL_IDS = Object.keys(CONTROL_BOUNDS).filter((id) => id !== "fixedDurationMs")
const REVIEW_SPEC = {
  defaultFixture: "seven",
  defaultCanvas: "wide",
...
check("continuous authoring edit becomes one reversible history step", () => {
  const causalControl = REVIEW_CONTROL_IDS.find((id) => id !== REVIEW_FEATURED_KEY)
  const initial = createAuthoringSnapshot({ fixture: "seven", canvas: "wide", controls: DEFAULTS, reducedMotionOverride: null })
...
const result = { scene: "proximity-orbit", passed: failures.length === 0, checks: vectors.length + 17, failures, observations }
