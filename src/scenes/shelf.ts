...
export const SHELF_EVALUATOR_AUTHORITY_SHA256 = "8520586750ccf2b51b60625dbb37d8bc54603b1c4c1db17b7aefe9661062fb60"
...
export function shelfSourceTimeMs(timeMs: number, durationMs: number, direction: "forward" | "reverse", terminal = false) {
    if (!Number.isFinite(timeMs) || !Number.isFinite(durationMs) || durationMs <= 0 || !["forward", "reverse"].includes(direction)) {
        throw new Error("Shelf source clock is invalid.")
    }
    return terminal ? clamp(timeMs, 0, durationMs) : positiveModulo(timeMs, durationMs)
}
...
function createShelfLayout(items: ShelfEvaluationItem[], parameters: ShelfParameters, stageWidth: number, stageHeight: number): ShelfLayout {
    ...
    const dimensions = items.map((item) => {
        const height = Math.min(nominalHeight, widthLimit / item.ratio)
        const width = height * item.ratio
        const baseLeanDeg = identityLean(item.id, parameters.leanAmount)
        return { width, height, baseLeanDeg }
    })
    const naturalTrackLength = dimensions.reduce((sum, value) => sum + value.width, 0) + items.length * parameters.gap
    const maximumWidth = dimensions.reduce((maximum, value) => Math.max(maximum, value.width), 0)
    const minimumLoop = stageWidth + maximumWidth * 2 + SHELF_OBSERVATION_GUARD * 2
    const trackLength = items.length <= 1 ? Math.max(1, naturalTrackLength) : Math.max(naturalTrackLength, minimumLoop)
    ...
    return {
        ...
        naturalTrackLength,
        effectiveGap,
        origin: items.length === 1 ? stageWidth / 2 - (cards[0]?.centre ?? 0) : stageWidth * 0.08,
        cards,
    }
}
...
export function inspectShelfSeams(items: ShelfEvaluationItem[], parameters: ShelfParameters, stageWidth: number, stageHeight: number): ShelfSeamRecord[] {
    ...
    return layout.cards.map((card) => {
        const normalizedBaseX = positiveModulo(layout.origin + card.centre, layout.trackLength)
        const exitBase = (normalizedBaseX + card.width / 2) / layout.trackLength
        const entryBase = (normalizedBaseX + layout.trackLength - card.width / 2 - stageWidth) / layout.trackLength
        ...
        const previousRightAtExit = currentBaseX - exitPhase * layout.trackLength + card.width / 2
        const nextLeftAtExit = currentBaseX - exitPhase * layout.trackLength + layout.trackLength - card.width / 2
        const nextLeftAtEntry = currentBaseX - nextEntryPhase * layout.trackLength + layout.trackLength - card.width / 2
        const previousRightAtEntry = currentBaseX - nextEntryPhase * layout.trackLength + card.width / 2
        return {
            id: card.id,
            ...
            previousRightAtExit: rounded(previousRightAtExit),
            nextLeftAtExit: rounded(nextLeftAtExit),
            nextLeftAtEntry: rounded(nextLeftAtEntry),
            previousRightAtEntry: rounded(previousRightAtEntry),
            seamOutsideVisibleStage: previousRightAtExit <= 1e-7 && nextLeftAtExit >= stageWidth
                && nextLeftAtEntry >= stageWidth - 1e-7 && previousRightAtEntry <= 0,
        }
    })
}
...
function alignPhase(index: number, layout: ShelfLayout, stageWidth: number) {
    return positiveModulo((layout.origin + layout.cards[index].centre - stageWidth / 2) / layout.trackLength, 1)
}
...
function finitePose(input: ShelfEvaluationInput, layout: ShelfLayout) {
    ...
    const spotlightAnchorIndex = spotlightFocusIndex ?? Math.min(2, input.items.length - 1)
    const finaleAnchorIndex = finaleFocusIndex ?? Math.max(0, input.items.length - 1)
    const spotlightPhase = alignPhase(spotlightAnchorIndex, layout, input.stageWidth)
    const finalePhase = alignPhase(finaleAnchorIndex, layout, input.stageWidth)
    ...
    if (normalized < 0.1) {
        phrase = "entry"
        assemblyOffset = input.stageWidth * 0.42 * (1 - minimumJerk(normalized / 0.1))
    } else if (normalized < 0.46) {
        phrase = "walking-to-spotlight"
        const progress = minimumJerk((normalized - 0.1) / 0.36)
        phase = positiveModulo(spotlightPhase * progress, 1)
        if (spotlightFocusIndex !== null) {
            currentFocusIndex = spotlightFocusIndex
            focusProgress = minimumJerk(clamp((normalized - 0.38) / 0.08, 0, 1))
        }
        ...
        const progress = minimumJerk((normalized - 0.72) / 0.14)
        phase = positiveModulo(spotlightPhase + positiveModulo(finalePhase - spotlightPhase, 1) * progress, 1)
        currentFocusIndex = progress < 0.5 ? spotlightFocusIndex : finaleFocusIndex
        focusProgress = currentFocusIndex === null ? 0 : 1 - Math.abs(progress - 0.5) * 2
        ...
        phrase = "exit"
        const progress = minimumJerk((normalized - 0.96) / 0.04)
        phase = finalePhase
        assemblyOffset = -input.stageWidth * 0.42 * progress
    ...
    return {
        phrase,
        phase,
        translationX: layout.origin - phase * layout.trackLength + assemblyOffset,
        wrapped: true,
        currentFocusIndex,
        focusProgress,
...
export function evaluateShelf(input: ShelfEvaluationInput): ShelfFrame {
    ...
    if (input.reducedMotion) {
        ...
        const anchor = focus ?? Math.min(2, input.items.length - 1)
        const phase = alignPhase(anchor, layout, input.stageWidth)
        pose = {
            phrase: "reduced-motion-settled",
            phase,
            translationX: layout.origin - phase * layout.trackLength,
            wrapped: true,
        ...
        pose = {
            ...
            translationX: layout.origin - temporal.phase * layout.trackLength,
            wrapped: true,
    ...
    const candidates: Array<ShelfRenderSlot & { zOrder: number }> = []
    for (const card of layout.cards) {
        ...
        for (const copyIndex of copyIndices) {
            ...
            const left = x - card.width / 2
            const right = x + card.width / 2
            ...
            candidates.push({
                ...
                zOrder: 10_000 + card.sourceIndex + Math.round(focusProgress * 2_000),
                opacity: 1,
                filter: "none",
                blend: "normal",
            })
    ...
    const ranked = [...candidates].sort((left, right) => Math.abs(left.x - input.stageWidth / 2) - Math.abs(right.x - input.stageWidth / 2)
        || left.sourceIndex - right.sourceIndex || left.copyIndex - right.copyIndex)
    const slots = ranked.slice(0, maxObservedNodes).sort((left, right) => left.zOrder - right.zOrder
        || left.sourceIndex - right.sourceIndex || left.copyIndex - right.copyIndex)
        .map(({ zOrder: _zOrder, ...slot }) => slot)
