...
export function effectiveShelfRatio(item: MediaItem, settings: ReelSettings) {
    ...
    if (!["auto", "global", "custom"].includes(item.aspectMode ?? "")) throw new Error("Shelf frame ratio intent is invalid.")
    if (item.aspectMode === "custom"
        && (!finiteInRange(item.ratioW ?? Number.NaN, 1, 10_000) || !finiteInRange(item.ratioH ?? Number.NaN, 1, 10_000))) {
        throw new Error("Shelf custom frame ratio is invalid.")
    }
    if (item.aspectMode === "global") {
        if (!["auto", "fixed"].includes(settings.ratioMode) || !["sixteenNine", "wide2576", "custom"].includes(settings.fixedRatio)) {
            throw new Error("Shelf global frame ratio intent is invalid.")
        }
        if (settings.ratioMode === "fixed" && settings.fixedRatio === "custom"
            && (!finiteInRange(settings.customRatioWidth, 1, 10_000) || !finiteInRange(settings.customRatioHeight, 1, 10_000))) {
            throw new Error("Shelf global custom ratio is invalid.")
        }
    }
    const crop = resolvedCrop(item)
    const cropped = crop.x !== 0 || crop.y !== 0 || crop.width !== 1 || crop.height !== 1
    let ratio = item.ratio
    if (cropped) ratio *= crop.width / crop.height
    else if (item.aspectMode === "custom") {
        ratio = (item.ratioW ?? 1) / (item.ratioH ?? 1)
    } else if (item.aspectMode === "global" && settings.ratioMode === "fixed") {
        ratio = settings.fixedRatio === "wide2576" ? 2576 / 1080
            : settings.fixedRatio === "custom" ? settings.customRatioWidth / settings.customRatioHeight : 16 / 9
    } else if (item.aspectMode !== "auto") {
        throw new Error("Shelf frame ratio intent is invalid.")
    }
    if (!finiteInRange(ratio, 0.2, 4)) throw new Error("Shelf effective frame ratio must be between 0.2 and 4.")
    return ratio
}
...
export function validateShelfRuntimeConfig(config: ReelConfig): ShelfRuntimeClock {
    ...
    if (!Array.isArray(config.items) || config.items.length > SHELF_MAX_ITEMS) throw new Error("Shelf ordered media intent is invalid.")
...
export function reconcileShelfConfig(config: ReelConfig): ReelConfig {
    ...
    if (config.items.length > SHELF_MAX_ITEMS) throw new Error(`Shelf supports at most ${SHELF_MAX_ITEMS} ordered media items.`)
